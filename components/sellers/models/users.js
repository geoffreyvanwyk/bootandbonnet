...
exports.user = {
	...
	update: function(userId, email, passwordHash, isVerified, callback) {
		...
		/**
	 * Updates the password of a row in the users database table, and returns that row, in the form of a user object, as the
	 * second argument to a callback function. The first argument to the callback function is an error object.
	 *
	 ...
	 *							When there is an error, the second argument is undefined.
	 */
	updatePassword: function(emailAddress, passwordHash, callback) {
		db.query("UPDATE users SET ? WHERE username = '".concat(emailAddress).concat("'"), {
			password: passwordHash
		}, function(err, result) {
				if (err) {
					return callback(err);
