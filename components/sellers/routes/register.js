...
/**
 * Displays the register form, to either add or edit a seller profile. If a 
 * seller is already logged-in, a new profile cannot added, so the function 
 ...
 * @returns {undefined}                  
 */
function showRegistrationForm(action, heading, method, btnCaption, emailError, sellerType,
...
/**
 * Responds to HTTP GET /seller/add. Displays the register form with all fields
 * blank. 
 * 
 * @param   {object}    request     An HTTP request object received from the 
 *									express.get() method.
 *
 * @param   {object}    response    An HTTP response object received from the 
 *									express.get() method.
 * 
 * @return  {undefined}
 */
...
/**
 * Responds to HTTP GET /seller/edit. Displays the register form with the 
 * fields prefilled with the logged-in seller's details. 
 * 
 * @param   {object}    request     An HTTP request object received from the 
 *									express.get() method.
 *
...
function edit(request, response) {
	...
	register.show('edit', 'Edit Seller', 'put', 'Save changes', '', ss.type,
			...
			ss.dealershipName, ss.streetAddress1, ss.streetAddress2, 
			ss.province, ss.town, ss.townId, ss.loggedIn, response);
}
